...
TEST 1: Player's hand should not change (+1 Card negated by playing Village)
TEST 2: Player's original hand has not been changed (except for playing Village)
TEST 3: Player's card count has not changed (i.e. deck + discard + hand + played stays the same)
TEST 4: Gamestate has not changed except +2 Actions
TEST 5: Other player variables have not changed
TEST 6: Village card should be played, i.e. added to playedCards array
...
int testOriginalHand(int player, int handpos, struct gameState* G, struct gameState* testG) {
  ...
  if (pass) printf("\tPASSED: Original cards in hand unchanged except for village played\n");
...
// will check that village was played, i.e. that it is appended to testG.playedCards
int testCardPlayed(int player, int handpos, struct gameState* G, struct gameState* testG) {
  if (testG->playedCardCount == G->playedCardCount + 1) {
    if (testG->playedCards[testG->playedCardCount - 1] == village)
      printf("\tPASSED: Village card was properly played, i.e. added to playedCard array\n");
    else
      printf("\t\u274CFAILED: Village card was not properly played, i.e. added to playedCard array\n");
  } 
  else printf("\t\u274CFAILED: Village card was not properly played, i.e. added to playedCard array\n");
  return 0;
}
...
int testGameState(struct gameState* G, struct gameState* testG) {
  ...
  if (G->numActions != testG->numActions - 2) {
    printf("\t\u274CFAILED: Gamestate numActions has not increased by 2\n");
    total_pass = 0;
  }
  ...
  for (int i = 0; i < G->playedCardCount; i++) {
    if (G->playedCards[i] != testG->playedCards[i]) {
      printf("\t\u274CFAILED: Played cards has changed aside from village\n");
      total_pass = 0;
    }
  }
...
int runTests(int player, int handpos, struct gameState* G, struct gameState* testG) {
  ...
  testCardPlayed(player, handpos, G, testG);            // ensures village card is played, i.e. added to playedCard array
...
int main() {
  ...
  villageEffect(&testG, player, handpos);   
  ...
  // TEST: discard should be empty (second case)
